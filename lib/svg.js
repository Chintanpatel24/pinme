...
function generateCard(repo, border = false, theme = 'light', showStats = true, width = 340, height = 112) {
  ...
  const starWidth = 18 + estimateTextWidth(stargazers_count);
  const forkWidth = 18 + estimateTextWidth(forks_count);

  const maxForkX = safeWidth - PADDING - forkWidth;
  const maxStarX = maxForkX - 20 - starWidth;

  const starX = Math.min(cursor, maxStarX);
  cursor = starX + starWidth + 20;
  const forkX = Math.min(cursor, maxForkX);
  ...
  <a href="${href}" xlink:href="${href}" target="_blank" rel="noopener noreferrer">
    ...
    <rect width="${safeWidth}" height="${safeHeight}" rx="6" fill="${themeColors.bg}" stroke="${themeColors.stroke}" stroke-width="1"/>
    ${borderBar}
...
export function generateSingleSVG(repo, border = false, theme = 'light', showStats = true, width = 340, height = 112) {
  const safeWidth = Math.min(Math.max(width || 340, 200), 1000);
  const safeHeight = Math.min(Math.max(height || 112, 70), 400);
  ...
  ${generateCard(repo, border, theme, showStats, safeWidth, safeHeight)}
</svg>`;
}

export function generateGridSVG(repos, cols = 2, border = false, theme = 'light', showStats = true, width = 340, height = 112) {
  const safeWidth = Math.min(Math.max(width || 340, 200), 1000);
  const safeHeight = Math.min(Math.max(height || 112, 70), 400);
  ...
  if (repos.length === 1) {
    return generateSingleSVG(repos[0], border, theme, showStats, safeWidth, safeHeight);
  }
