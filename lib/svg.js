...
function generateCard(repo, border = false) {
  const {
    full_name,
    description,
    language,
  ...
  const descLines = wrapDescription(cleanDescription, 46);
  const name = truncateText(cleanFullName, 36);
  ...
  const descPart = descLines.length > 0
    ? descLines.map((line, i) =>
        `<text x="${PADDING}" y="${57 + i * 16}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(line)}</text>`
      ).join('')
    : '';
  ...
  if (language) {
    languagePart = `<circle cx="${cursor + 6}" cy="${metaY - 4}" r="6" fill="${langColor}"/>` +
      `<text x="${cursor + 18}" y="${metaY}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(language)}</text>`;
    cursor += 18 + estimateTextWidth(language) + 20;
  }

  const starX = Math.min(cursor, 202);
  cursor = starX + 18 + estimateTextWidth(stargazers_count) + 20;
  const forkX = Math.min(cursor, 274);

  const borderBar = border
    ? `<rect x="1" y="1" width="5" height="${CARD_HEIGHT - 2}" rx="2" fill="${langColor}"/>`
    : '';
  ...
  <a href="${href}" xlink:href="${href}" target="_blank" rel="noopener noreferrer">
    ...
    ${borderBar}
    <g transform="translate(${PADDING}, 21)">
      ${REPO_ICON}
    </g>
    <text x="${PADDING + 22}" y="35" fill="#0969da" font-size="14" font-weight="600" font-family="${escapeXml(FONT_FAMILY)}">${name}</text>
    ${descPart}
    ${languagePart}
    <g transform="translate(${starX}, ${iconY})">
      ${STAR_ICON}
    </g>
    <text x="${starX + 18}" y="${metaY}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(stargazers_count)}</text>
    <g transform="translate(${forkX}, ${iconY})">
      ${FORK_ICON}
    </g>
    <text x="${forkX + 18}" y="${metaY}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(forks_count)}</text>
  </a>
`;
}

export function generateSingleSVG(repo, border = false) {
  const title = escapeXml(repo.full_name || 'GitHub repository');
  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${CARD_WIDTH}" height="${CARD_HEIGHT}" viewBox="0 0 ${CARD_WIDTH} ${CARD_HEIGHT}" role="img" aria-label="${title}">
  ${generateCard(repo, border)}
</svg>`;
}

export function generateGridSVG(repos, cols = 2, border = false) {
  if (repos.length === 0) {
    return generateErrorSVG('No repositories to display.');
  }
  if (repos.length === 1) {
    return generateSingleSVG(repos[0], border);
  }
