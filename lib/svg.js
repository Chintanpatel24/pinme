...
function generateCard(repo) {
  ...
  const descPart = descLines.length > 0
    ? descLines.map((line, i) =>
        `<text x="${PADDING}" y="${58 + i * 16}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(line)}</text>`
      ).join('')
    : '';
  ...
  const languagePart = language
    ? `<circle cx="${PADDING}" cy="${langDotY}" r="6" fill="${langColor}"/>` +
      `<text x="${PADDING + 14}" y="${langTextY}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(language)}</text>`
    : '';
  ...
  return `
    ...
    <rect width="4" height="${CARD_HEIGHT}" fill="${langColor}"/>
    <g transform="translate(${PADDING}, 20)">
      ${REPO_ICON}
    </g>
    <text x="${PADDING + 22}" y="34" fill="#0969da" font-size="14" font-weight="600" font-family="${escapeXml(FONT_FAMILY)}">${name}</text>
    ${descPart}
    ${languagePart}
    <g transform="translate(${starX}, ${iconY})">
      ${STAR_ICON}
    </g>
    <text x="${starX + 18}" y="${langTextY}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(stargazers_count)}</text>
    <g transform="translate(${forkX}, ${iconY})">
      ${FORK_ICON}
    </g>
    <text x="${forkX + 18}" y="${langTextY}" fill="#656d76" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(forks_count)}</text>
}

export function generateSingleSVG(repo) {
  return `<svg width="${CARD_WIDTH}" height="${CARD_HEIGHT}">
  ${generateCard(repo)}
</svg>`;
}
...
export function generateErrorSVG(message) {
  ...
  return `<svg width="400" height="80">
...
amily="${escapeXml(FONT_FAMILY)}">${escapeXml(line)}</text>`
  ).join('')}
</svg>`;
}
ily="${escapeXml(FONT_FAMILY)}">${escapeXml(line)}</text>`
  ).join('')}
</svg>`;
}
amily="${escapeXml(FONT_FAMILY)}">${escapeXml(line)}</text>`
