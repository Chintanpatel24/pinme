...
function generateCard(repo, border = false, theme = 'light', showStats = true) {
  const {
    full_name,
    description,
    language,
  ...
  const starPart = showStats
    ? `<g transform="translate(${starX}, ${iconY})" fill="${themeColors.icon}">
      ${STAR_ICON}
    </g>
    <text x="${starX + 18}" y="${metaY}" fill="${themeColors.text}" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(stargazers_count)}</text>`
    : '';

  const forkPart = showStats
    ? `<g transform="translate(${forkX}, ${iconY})" fill="${themeColors.icon}">
      ${FORK_ICON}
    </g>
    <text x="${forkX + 18}" y="${metaY}" fill="${themeColors.text}" font-size="12" font-family="${escapeXml(FONT_FAMILY)}">${escapeXml(forks_count)}</text>`
    : '';
  ...
  <a href="${href}" xlink:href="${href}" target="_blank" rel="noopener noreferrer">
    ...
    <rect width="${CARD_WIDTH}" height="${CARD_HEIGHT}" rx="6" fill="${themeColors.bg}" stroke="${themeColors.stroke}" stroke-width="1"/>
    ${borderBar}
...
export function generateSingleSVG(repo, border = false, theme = 'light', showStats = true) {
  ...
  ${generateCard(repo, border, theme, showStats)}
</svg>`;
}

export function generateGridSVG(repos, cols = 2, border = false, theme = 'light', showStats = true) {
  if (repos.length === 0) {
    return generateErrorSVG('No repositories to display.');
  }
  if (repos.length === 1) {
    return generateSingleSVG(repos[0], border, theme, showStats);
  }
