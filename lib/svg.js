...
function generateCard(repo, border = false, theme = 'light') {
  const {
    full_name,
    description,
    language,
...
export function generateSingleSVG(repo, border = false, theme = 'light') {
  ...
  ${generateCard(repo, border, theme)}
</svg>`;
}

export function generateGridSVG(repos, cols = 2, border = false, theme = 'light') {
  if (repos.length === 0) {
    return generateErrorSVG('No repositories to display.');
  }
  if (repos.length === 1) {
    return generateSingleSVG(repos[0], border, theme);
  }
