...
function generateCard(repo) {
  ...
  const descLines = wrapDescription(description, 46);
  const name = truncateText(full_name, 36);
  const href = escapeXml(html_url || '#');
  const title = escapeXml(full_name || 'GitHub repository');
  const cardContent = escapeXml(`${full_name}${description ? `: ${description}` : ''}`);
...
export function generateGridSVG(repos, cols = 2) {
  if (repos.length === 0) {
